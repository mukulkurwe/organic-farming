...
const PAYMENT_STATUS = {
  paid:    { label: "Paid",    icon: CheckCircle, bg: "bg-emerald-50",  text: "text-emerald-700", border: "border-emerald-200" },
  pending: { label: "Pending", icon: Clock,       bg: "bg-amber-50",    text: "text-amber-700",   border: "border-amber-200"  },
  partial: { label: "Partial", icon: AlertCircle, bg: "bg-blue-50",     text: "text-blue-700",    border: "border-blue-200"   },
};
...
function StatCard({ label, value, icon: Icon, color }) {
  return (
    <div className={`bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4 border-l-4 ${color}`}>
...
export default function TransactionsPage() {
  ...
  const visible = transactions.filter(t => {
    ...
    const matchQ = !q || (t.buyer_name || "").toLowerCase().includes(q) || (t.crop_name || "").toLowerCase().includes(q);
    const matchP = !filterPayment || t.payment_status === filterPayment;
    return matchQ && matchP;
  });

  const totalRevenue = visible.reduce((s, t) => s + Number(t.total_amount || 0), 0);
  const paidRevenue  = visible.filter(t => t.payment_status === "paid").reduce((s, t) => s + Number(t.total_amount || 0), 0);
  const pendingCount = visible.filter(t => t.payment_status === "pending").length;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => router.push("/farmer/sales/transactions/new")}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Revenue" value={fmtRupee(totalRevenue)} icon={IndianRupee} color="border-l-emerald-500" />
          <StatCard label="Paid Revenue"  value={fmtRupee(paidRevenue)}  icon={CheckCircle}  color="border-l-blue-500" />
          <StatCard label="Transactions"  value={visible.length}         icon={Receipt}       color="border-l-violet-500" />
          <StatCard label="Pending"       value={pendingCount}           icon={Clock}         color="border-l-amber-500" />
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="p-16 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1 mb-5">Record your first sale to track revenue</p>
              <button onClick={() => router.push("/farmer/sales/transactions/new")}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {["Date", "Buyer", "Crop", "Qty Sold", "Amount", "Mode", "Payment", "Farm"].map(h => (
                  ...
                  {visible.map(tx => (
                    <tr key={tx.id} className="hover:bg-gray-50 transition">
                      <td className="px-5 py-4 text-sm text-gray-700 whitespace-nowrap">
                        {new Date(tx.sale_date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900">{tx.buyer_name}</p>
                            {tx.buyer_type && <p className="text-xs text-gray-400 capitalize">{tx.buyer_type.replace("_", " ")}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4 text-sm text-gray-700">{tx.crop_name || "—"}</td>
                      <td className="px-5 py-4 text-sm text-gray-700 whitespace-nowrap">
                        {Number(tx.quantity_sold).toLocaleString("en-IN")} {tx.unit || "kg"}
                      </td>
                      <td className="px-5 py-4 text-sm font-semibold text-gray-900 whitespace-nowrap">{fmtRupee(tx.total_amount)}</td>
                      <td className="px-5 py-4 text-sm text-gray-600">{PAYMENT_MODE[tx.payment_mode] || tx.payment_mode}</td>
                      <td className="px-5 py-4"><PaymentBadge status={tx.payment_status} /></td>
                      <td className="px-5 py-4 text-sm text-gray-500">{tx.farm_name || "—"}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50 border-t border-gray-200">
                    <td colSpan={4} className="px-5 py-3.5 text-sm font-semibold text-gray-700">Total</td>
                    <td className="px-5 py-3.5 text-sm font-bold text-emerald-700">{fmtRupee(totalRevenue)}</td>
                    <td colSpan={3} />
                  </tr>
                </tfoot>
              </table>
