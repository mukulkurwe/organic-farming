...
import { useEffect, useMemo, useState } from "react";
// ❌ import { apiGet } from "@/lib/api";
import api from "@/services/api"; // ✅ shared axios client pointing to Render
  ...
  useEffect(() => {
    async function loadMonth() {
      try {
        ...
        // ✅ NEW: use shared axios client -> hits Render in production
        const res = await api.get("/supervisor/activities/month", {
          params: { farm_id: farmId, month: currentMonth },
        });
        const data = res.data;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* Workers */}
          <select className="border border-gray-400 bg-white text-gray-800 rounded-lg px-4 py-2 text-sm font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            <option className="text-gray-800">All Workers</option>
            <option className="text-gray-800">Ram</option>
            <option className="text-gray-800">Sita</option>
            <option className="text-gray-800">Raju</option>
            ...
            <div className="border border-gray-300 rounded-lg overflow-hidden">
              ...
              <div>
                {weeks.map((week, wi) => (
                  <div
                    key={wi}
                    className="grid grid-cols-7 border-t border-gray-300 bg-white "
                  >
                    {week.map((cell, ci) => {
                      if (!cell) {
                        return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <ChartCard title="Activity Type Distribution (This Month)">
            <p className="text-xs text-gray-600 mb-3">
              Pie Chart: Sowing 10% | Irrigation 40% | Pest 20% | Fertilizer 15%
              | Others 15%
            </p>
            <div className="h-48 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center text-sm text-gray-500 bg-gray-50">
              Pie chart placeholder – connect Recharts
